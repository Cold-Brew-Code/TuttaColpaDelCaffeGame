...
public class TalkObserver implements GameObserver {
    ...
    public String update(GameDescription description, ParserOutput parserOutput) {
        ...
        // lista npc 1 e ha specificato con chi vuole parlare ma non è necessari
        if (parserOutput.getCommand().getType() == CommandType.TALK_TO) {
            if (description.getCurrentRoom().getNPCs().size() == 1 && parserOutput.getObject() != null) {
                msg.append("Per parlare con qualcuno scrivere 'Parla'! ");
            }// lista npc 1 e non ha specificato con chi vuole paralre
            else if (description.getCurrentRoom().getNPCs().size() == 1 && parserOutput.getObject() == null) {
            ...
            else if (parserOutput.getCommand().getType() == CommandType.TALK_TO && description.getCurrentRoom().getNPCs().size() > 1 && parserOutput.getObject() != null) {
